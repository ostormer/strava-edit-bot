...
public class MockActivityService : IActivityService
{
    ...
    [
        new Activity("Morning Run", "A nice run in the park", "Run", DateTime.Now.AddDays(-1), 5.0, TimeSpan.FromMinutes(30)) { Id = 1 },
        new Activity("Evening Ride", "A relaxing bike ride", "Ride", DateTime.Now.AddDays(-2), 20.0, TimeSpan.FromMinutes(60)) { Id = 2 }
    ];
